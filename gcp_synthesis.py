...
class GCP_Synthesis():
    def __init__(self):
        self.base_logo = 'bạn đang nghe truyện audio trên kênh s truyện 2 4 7. nếu thấy hay đừng quên bấm like và sub cribe kênh nhé'
        self.gcp_headers = {
        ...
        self.gcp_url = 'https://cxl-services.appspot.com/proxy?url=https://us-central1-texttospeech.googleapis.com/v1beta1/text:synthesize&token=0'

    # format text
    def formatText(self,text='', is_number = True):
        '''
        Format text 
        '''
    ...
    # format payload
    def format_payload(self, text='',pitch=-0.3, speakingRate=1.05, isFormat = True):
        '''
    ...
    def handle_request(self, url='', headers = '', payload=''):
        try:
    ...
    # combine audio
    def combine_audio(self, audio_path='', audio_name='', audio_data=[]):
        try:
            audio_content = b''
    ...
    def gcp_text_to_speech(self, url='', chapter_name='', chapter_data='', event=None):
        if event.is_set():
            logging.info(f'Stopping, name={chapter_name}')
            return
        #  
        try:
            # 
            chapter_contents = chapter_data + self.base_logo
            ...
            for current_count, current_content in enumerate(chapter_lists):
                gcp_payload = self.format_payload(current_content)
                gcp_request_data = self.handle_request(url=self.gcp_url, headers=self.gcp_headers, payload=gcp_payload)
                if gcp_request_data['status'] == True:
                    list_audio_data.append(gcp_request_data['result'])
                    logging.info(f"\r Downloading file: {round(current_count/total_list *100)}%")
                    ...
                    for child_count, child_content in enumerate(child_contents):
                        child_gcp_payload = self.format_payload(child_content)
                        gcp_request_data = self.handle_request(url=self.gcp_url, headers=self.gcp_headers, payload=child_gcp_payload)
                        if gcp_request_data['status'] == True:
                            list_audio_data.append(gcp_request_data['result'])
                            time.sleep(1)
                            logging.info(f"\r Downloading child: {round((child_count/child_list)*100)}%")
                        else:
                            logging.error(gcp_request_data['message'])
                            event.set()
                            return False       
            if not list_audio_data:
                ...
                self.combine_audio(audio_path=audio_path, audio_name=audio_name, audio_data=list_audio_data)
    ...
    # multiple
    def multiple_speech_synthesis(self):
        list_process = []
        event = Event()
        with ThreadPoolExecutor(3) as executor:
            for chapter_name in sorted_alphanumeric(os.listdir(inputFolders)):
                ...
                with open(chapter_path, 'r', encoding='utf-8') as f:
                    chapter_data = f.read()
                process = executor.submit(self.gcp_text_to_speech, *
                                            [self.gcp_url,chapter_name, chapter_data, event])
                list_process.append(process)
            return [process.result() for process in list_process]
def handle_process():
    gcp_synthesis = GCP_Synthesis()
    process = Process(target=gcp_synthesis.multiple_speech_synthesis)
    process.start()
    time.sleep(100)
    process.terminate()
